...
from .baseclass import BaseTimePDE
...
class Heat2D_VaryingCoef(baseclass.BaseTimePDE):
    ...
    def __init__(self, datapath="ref/heat_darcy.dat", bbox=[0, 1, 0, 1, 0, 5], A=200, m=(1, 5, 1)):
        ...
        def coef(x):
            return torch.Tensor(
                interpolate.griddata(self.heat_2d_coef[:, 0:2], self.heat_2d_coef[:, 2],
                                     x.detach().cpu().numpy()[:, 0:2], method='nearest')
            ).unsqueeze(dim=1)
        ...
        def heat_pde(x, u):
            ...
            def f(x):
                return A * torch.sin(m[0] * torch.pi * x[:, 0:1]) * torch.sin(m[1] * torch.pi * x[:, 1:2]) * torch.sin(
                    m[2] * torch.pi * x[:, 2:3])
        ...
        def boundary_xb(x, on_boundary):
            return on_boundary and (np.isclose(x[0], bbox[0]) or np.isclose(x[0], bbox[1]) or np.isclose(x[1], bbox[
                2]) or np.isclose(x[1], bbox[3]))
...
class Heat2D_Multiscale(baseclass.BaseTimePDE):
    ...
    def __init__(
            ...
            # pde_coef=(1 / np.square(np.pi), 1 / np.square(np.pi)),
        ...
        # self.output_config = [{'name': 'x'}, {'name': 'y'}, {'name': 't'}]
        ...
        def initial_conditions(x, on_initial):
            return on_initial

        def boundary_left(dim, on_boundary):
            return on_boundary and np.isclose(dim[0], bbox[0])

        def boundary_right(dim, on_boundary):
            return on_boundary and np.isclose(dim[0], bbox[1])

        def boundary_bottom(dim, on_boundary):
            return on_boundary and np.isclose(dim[1], bbox[2])

        def boundary_upper(dim, on_boundary):
            return on_boundary and np.isclose(dim[1], bbox[3])
...
#
#         def ref_sol(xt):
#             result = np.sin(init_coef[0] * xt[:, 0:1]) * np.sin(init_coef[1] * xt[:, 1:2]) * \
#                    np.exp(-(pde_coef[0]*init_coef[0]**2 + pde_coef[1]*init_coef[1]**2) * xt[:, 2:3])
#
#             return result
#
    ...
    def __init__(self, datapath="ref/heat_longtime.dat", bbox=[0, 1, 0, 1, 0, 100], k=1, m1=4, m2=2):
        ...
        def pde(x, u):
            ...
            return [
                ...
                (1 + 2 * torch.sin(x[:, 2:3] * np.pi / 4)) * torch.sin(m1 * np.pi * x[:, 0:1]) * torch.sin(
                    m2 * np.pi * x[:, 1:2])
            ]
...
class Heat1D(BaseTimePDE):
    def __init__(self, datapath='ref/heat_1d.dat', geom = [0.0, 1.0], time=[0.0, 0.2]):
        super().__init__()
        ...
        def boundary_left(dim, on_boundary):
            return on_boundary and np.isclose(dim[0], self.bbox[0])

        def boundary_right(dim, on_boundary):
            return on_boundary and np.isclose(dim[0], self.bbox[1])

        self.add_bcs([{
            'component': 0,
            'function': (lambda _: 50),
            'bc': (lambda _, on_initial: on_initial),
            'type': 'ic',
        }, {
            'component': 0,
            'function': (lambda _: 100),
            'bc': boundary_left,
            'type': 'dirichlet'
        }, {
            'component': 0,
        ...
        #     'component': 0,
        #     'function': (lambda _: 100),
        #     'bc': (lambda _, on_boundary: boundary_left),
        #     'type': 'dirichlet',
        # }, {
        #     'component': 0,
        #     'function': (lambda _: 0),
        #     'bc': (lambda _, on_boundary: boundary_right),
        #     'type': 'dirichlet',
        # }
...
class HeatND(baseclass.BaseTimePDE):
    ...
    def __init__(self, dim=1, T=1):
        ...
        self.geom = dde.geometry.Hypercube([-1] * dim, [1] * dim)
